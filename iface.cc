...
void Firewall::ProcessTarget(processed_rule * pr, rule_tuple * tup, rule_tuple * &stack)
{
	rule_tuple *newTup;
	int val;
...
void Firewall::ProcessFlags(processed_rule * pr, rule_tuple * tup, rule_tuple * &stack)
{
	int i;
...
void Firewall::ProcessState(processed_rule * pr, rule_tuple * tup, rule_tuple * &stack)
{
	int state = pr->state;
...
void Firewall::ProcessDport(processed_rule * pr, rule_tuple * tup, rule_tuple * &stack)
{
	port_range *cur;
...
void Firewall::ProcessSport(processed_rule * pr, rule_tuple * tup, rule_tuple * &stack)
{
	port_range *cur;
...
void Firewall::ProcessProt(processed_rule * pr, rule_tuple * tup, rule_tuple * &stack)
{
	switch (pr->protocol) {
		case 'i':
...
void Firewall::ProcessDest(processed_rule * pr, rule_tuple * tup, rule_tuple * &stack)
{
...
void Firewall::ProcessSource(processed_rule * pr, rule_tuple * tup, rule_tuple * &stack)
{
	...
	if (pr->from->next != NULL) {
		...
		tup->low[17] = pr->from->next->low % 256;
		tup->low[18] = (pr->from->next->low / 256) % 256;
		tup->low[19] = ((pr->from->next->low / 256) / 256) % 256;
		tup->low[20] =
			((signed int) (((pr->from->next->low / 256) / 256) / 256)) % 256;
		tup->high[17] = pr->from->next->high % 256;
		tup->high[18] = (pr->from->next->high / 256) % 256;
		tup->high[19] = ((pr->from->next->high / 256) / 256) % 256;
		tup->high[20] =
...
void Firewall::ProcessChain(chain ** chain_array, mdd_handle inMDD, rule_tuple * tup, mdd_handle & outMDD, mdd_handle & logMDD)
{
...
void Firewall::AssembleChains(chain ** chain_array, chain * chain, mdd_handle & outMDD, mdd_handle & logMDD)
{
	...
	high[5] = 1;					  // ACK flag
	low[6] = 0;
	high[6] = 1;					  // any URG flag
	low[7] = 0;
	high[7] = 3;					  // Any state
	low[8] = 0;
	high[8] = 255;					  // Any destination port
	low[9] = 0;
	high[9] = 255;
	low[10] = 0;
	high[10] = 255;				  // Any source port
	low[11] = 0;
	high[11] = 255;

	low[12] = 0;
	high[12] = 2;					  // Any Protocol
	low[13] = 0;
	high[13] = 255;				  // Any destination IP
	low[14] = 0;
	high[14] = 255;
	low[15] = 0;
	high[15] = 255;
