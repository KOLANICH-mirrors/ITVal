...
void BreakNAT(char *str, nat_range * &NATRange, char prot, Firewall * FW)
{
	...
	NATRange->low[16] = (((NATRange->addys.low / 256) / 256) / 256) % 256;
	NATRange->low[15] = ((NATRange->addys.low / 256) / 256) % 256;
	NATRange->low[14] = (NATRange->addys.low / 256) % 256;
	NATRange->low[13] = NATRange->addys.low % 256;

	NATRange->high[16] = (((NATRange->addys.high / 256) / 256) / 256) % 256;
	NATRange->high[15] = ((NATRange->addys.high / 256) / 256) % 256;
	NATRange->high[14] = (NATRange->addys.high / 256) % 256;
	NATRange->high[13] = NATRange->addys.high % 256;
	...
	if (prot == 'a') {
		...
		NATRange->low[12] = NATRange->high[12] = 0;
	}
	else if (prot == 'u') {
		NATRange->low[12] = NATRange->high[12] = 1;
	}
	else if (prot == 't') {
		NATRange->low[12] = NATRange->high[12] = 2;
	}
	...
	NATRange->low[11] = NATRange->low[10] = 0;
	NATRange->high[11] = NATRange->high[10] = 255;

	NATRange->low[10] = NATRange->low[10] = 0;
	NATRange->high[10] = NATRange->high[10] = 255;

	NATRange->low[9] = (NATRange->ports.port1 / 256) % 256;
	NATRange->low[8] = NATRange->ports.port1 % 256;

	NATRange->high[9] = (NATRange->ports.port2 / 256) % 256;
	NATRange->high[8] = NATRange->ports.port2 % 256;
}
