...
unsigned int fddl_forest::hashnode(level k, node_idx p)
{
	node   *nodeP;
	unsigned int val;
...
void    fddl_forest::ToggleSparsity(bool SparseSwitch)
{
...
void    fddl_forest::SetGarbageCollection(int alg, int threshold)
{
	garbage_alg = alg;
...
node_idx fddl_forest::InternalRestrict(level k, node_idx p, node_idx q)
{
	...
	if (!IS_SPARSE(nodeP) && !IS_SPARSE(nodeQ)) {
		for (arc_idx i = 0; i < (psize > qsize ? psize : qsize); i++) {
			node_idx u = InternalRestrict(k - 1,
...
void    fddl_forest::DeleteNode(level k, node_idx p)
{
	if (k < 1)
		return;
...
void    fddl_forest::DeleteDownstream(level k, node_idx p)
{
	if (p == 0)
		return;
	node   *nodeP = &FDDL_NODE(k, p);
...
void    fddl_forest::CompactTopLevel()
{
		...
		if (!IS_DELETED(nodeI)) { //If it's not deleted,
			...
			if (nodeI->flags & SPARSE) {	//If it's sparse, compact
				for (j = 0; j < nodeI->size; j++) {	//its arcs this way.
					...
					(*(*arc_temp_array)[tail[K] + 1]) = SPARSE_ARC(K, nodeI, j);
					tail[K] += 2;
				}
			}
...
void    fddl_forest::Compact(level k)
{
	compactions++;
	...
	for (i = 1; i < last[k]; i++) {	//Scan throught the NODE list (except node 0)
		...
		if (nodeI->in > 0) {		  //If it's not deleted, Copy it
			(*(*node_remap_array[k])[i]) = numvalidnodes;	//Save the "old" idx of the node
			(*(*node_temp_array)[numvalidnodes]) = (*nodeI);	//into a temp array.  
...
void    fddl_forest::FlushCaches(level k)
{
	MaxCache[k]->Clear();
...
void    fddl_forest::SetArc(level k, node_idx p, arc_idx i, node_idx j)
{
...
int     fddl_forest::UnpackNode(level k, arc_idx p, int *&fullarray)
{
#ifdef FDDL_DEBUG
...
void    fddl_forest::PrintStates(node_idx root)
{
...
void    fddl_forest::PrintVals(mdd_handle root, level k)
{
...
void fddl_forest::PrintRanges(mdd_handle root, level* mask){
   node_idx newRoot;
   print_node* p;
   print_node* stack;
   int* low;
...
void fddl_forest::PrintAddy(mdd_handle root, level k)
{
   node_idx newRoot;
   level mask[K+1];
...
void    fddl_forest::PrintPort(mdd_handle root, level k)
{
	node_idx newRoot;
	portset *p;
...
void fddl_forest::PrintRanges(level k, node_idx p, level* mask, print_node*& stack, int* low, int* high){
   node_idx child;
   node* nodeP;
   node_idx iVal;
   node_idx lastVal;
...
void    fddl_forest::PrintAddy(level k, node_idx p, int *vals, int depth)
{
	int i;
	int lastVal;
...
int fddl_forest::PrintPort(level k, node_idx p, int highByte, level cutoff,
									portset * ps)
{
	int     i;
	int     flag;
...
void    fddl_forest::PrintStates(level k, node_idx p, int *stateArray)
{
	arc_idx i;
	node   *nodeP;
...
void    fddl_forest::PruneMDD(mdd_handle p)
{
...
void    fddl_forest::PruneMDD(node_idx p)
{
	for (level k = K; k > 0; k--)
		PruneCache[k]->Clear();
...
void    fddl_forest::InternalPruneMDD(level k, node_idx p, int flag)
{
	node   *nodeP;
	int result;
...
void    fddl_forest::PrintMDD(int top, int bottom)
{
...
void    fddl_forest::SaveMDD(char *filename)
{
...
void    fddl_forest::LoadMDD(char *filename)
{
...
void    fddl_forest::PrintByRange(mdd_handle h)
{
	val_list *vals[K];

	for (level k = K; k > 0; k--)
		vals[k] = NULL;
	InternalPrintByRange(K, h.index, vals);
}

void
	   fddl_forest::InternalPrintByRange(level k, node_idx p, val_list ** vals)
{
	if (k == 0) {
		val_list *cur;
	...
	node_idx child;
	arc_idx *unassigned;
	unassigned = new arc_idx[nodeP->size];

	for (arc_idx i = 0; i < nodeP->size; i++) {
		unassigned[i] = i;
	}
	int     lastVal = nodeP->size - 1;

	while (unassigned[0] != -1) {
		for (arc_idx i = 0; i < nodeP->size; i++) {
			if (unassigned[i] == -1)
				break;
			int     val = FDDL_ARC(k, nodeP, i);

			for (arc_idx j = i; j < nodeP->size; j++) {
				if (unassigned[i] == -1)
					break;
				val_list *newVal;
				...
				newVal->next = vals[k];
				vals[k] = newVal;
				unassigned[j] = unassigned[lastVal];
				unassigned[lastVal] = (-1);
				lastVal--;
			}
			InternalPrintByRange(k - 1, child, vals);
		}
	}
}
