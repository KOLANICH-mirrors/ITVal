...
node_idx 
fddl_forest::InternalMax(level k, node_idx p, node_idx q)
{
	//Easy Terminal Cases
	if (p == 0 || p == q)
		return q;
...
node_idx 
fddl_forest::InternalRestrict(level k, node_idx p, node_idx q)
{
	//Easy Terminal Cases
	if (p == 0 || p == q)
		return q;
...
node_idx 
fddl_forest::CheckIn(level k, node_idx p)
{
	node   *nodeP;
...
void    
fddl_forest::PrintPort(mdd_handle root, level k)
{
	node_idx newRoot;
	portset *p;
...
void 
fddl_forest::PrintRanges(level k, node_idx p, level* mask, print_node*& stack, 
		int* low, int* high)
{
   node_idx child;
   node* nodeP;
...
int 
fddl_forest::PrintPort(level k, node_idx p, int highByte, level cutoff, 
		portset * ps)
{
	int     i;
	int     flag;
...
node_idx 
fddl_forest::ProjectVals(level k, node_idx p, level cutoff)
{
	node   *nodeP;
	node_idx result;
	node_idx flag;
...
node_idx 
fddl_forest::Projection(level k, node_idx p, level* mask)
{
	node   *nodeP;
	node_idx result;
	node_idx flag;
...
void
fddl_forest::PrintByRange(mdd_handle h)
{
	val_list *vals[K];
