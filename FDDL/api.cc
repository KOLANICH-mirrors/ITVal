...
int 
	 ...
	 fddl_forest::Assign(mdd_handle root, int *low, int *high,
								mdd_handle & result)
{
	level   k;
...
int
	...
	fddl_forest::Select(mdd_handle root, int num_chains,
...
int fddl_forest::ProjectOnto(mdd_handle p, mdd_handle q, mdd_handle &
result) { 
...
int
	...
	fddl_forest::Combine(mdd_handle root, mdd_handle root2,
...
int
	...
	fddl_forest::QueryIntersect(mdd_handle root, mdd_handle root2,
										 mdd_handle & result)
{
	if (root.index < 0)
		return INVALID_MDD;
...
int
	   ...
	   fddl_forest::SNAT(mdd_handle root, nat_tuple * pnr, mdd_handle & result)
{
	if (root.index < 0)
		return INVALID_MDD;
	if (pnr == NULL)
...
int fddl_forest::DNAT(mdd_handle root, nat_tuple * pnr, mdd_handle & result)
{
	if (root.index < 0)
		return INVALID_MDD;
	if (pnr == NULL)
...
int fddl_forest::NETMAP(mdd_handle root, nat_tuple * pnr, mdd_handle & result)
{
	if (root.index < 0)
		return INVALID_MDD;
	if (pnr == NULL)
...
node_idx fddl_forest::InternalSNAT(level k, node_idx p, node_idx q, nat_tuple * pnr)
{
	...
	if (k < 10) {					  //If we're beyond the source port
	...
	if (k >= 17 || k < 12) {	  //If it's a source address node
...
node_idx fddl_forest::InternalDNAT(level k, node_idx p, node_idx q, nat_tuple * pnr)
{
	//Node p is the original address.
	//Node q is the NATTed address.
	arc_idx i;
	...
	if (k < 8) {					  //If we're beyond the destination and port
		//return the node pointed to by the NATted address.
		result = CheckIn(k, q);
		return result;
	}
...
node_idx fddl_forest::InternalNMAP(level k, node_idx p, node_idx q, nat_tuple * pnr)
{  //MODIFY!
	//Node p is the original address.
	//Node q is the NATTed address.
...
node_idx
	fddl_forest::InternalSelect(level k, node_idx p, int
										 num_chains, node_idx *child_array)
{
...
node_idx fddl_forest::InternalReplaceStrict(level k, node_idx p, node_idx q)
{
	if (p == 0) {
		return 0;
	}
...
node_idx
        fddl_forest::InternalQIntersect(level k, node_idx p, node_idx q)
{
	arc_idx i;
	node_idx result, u;
	node   *nodeP, *nodeQ;
...
int     fddl_forest::Max(mdd_handle a, mdd_handle b, mdd_handle & result)
{
