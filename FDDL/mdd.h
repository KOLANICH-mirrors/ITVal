...
#   include "errors.h"
#   include "mddtypes.h"
#   include "datastructures/caches.h"
#   include "datastructures/dynarray.h"
#   include "datastructures/uniquetable.h"
#   include "datastructures/portset.h"
...
class   fddl_forest {
 ...
 public:
	...
	fddl_forest(int numlevels, int *maxvals) {
		...
		//Level K is set up seperately, because we expect it to be
		//significantly smaller than the internal levels.  Especially
		//when we are only creating and manipulating a single MDD.
   ...
   node_idx Projection(level k, node_idx p, level* mask);
	...
	int     InternalSNAT(level k, node_idx p, node_idx q, nat_tuple * pnr);
	int     InternalDNAT(level k, node_idx p, node_idx q, nat_tuple * pnr);
	int     InternalNMAP(level k, node_idx p, node_idx q, nat_tuple * pnr);
